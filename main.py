...
if not BOT_TOKEN:
    raise RuntimeError(
        "BOT_TOKEN is not set. Create a .env file next to main.py with:\n"
                            ...
                            for fld in ("s", "e"):
                                ...
                                if isinstance(v, list) and len(v) >= 3:
                                    if rgba_close(v, target):
                                        return True
        for v in obj.values():
            if color_exists(v, target):
                return True
    elif isinstance(obj, list):
        for item in obj:
            if color_exists(item, target):
                return True
    return False
...
def make_text_layers(text: str, color: list) -> list:
    ...
    fill = Fill()
    fill.color.value = Color(r, g, b, a)
...
def count_lotties() -> int:
    """Counts how many numbered .json base animations are in LOTTIES_DIR."""
    return len(list(LOTTIES_DIR.glob("*.json")))
...
def select_kb():
    ...
    return InlineKeyboardMarkup(
        inline_keyboard=[
            [
                InlineKeyboardButton(text=f"🎯 FULL ({total} ta)", callback_data="full"),
            ]
        ]
    )
...
async def cb_type(call: CallbackQuery, state: FSMContext):
    ...
    await call.message.edit_text(
        "Which animations do you want to modify?", reply_markup=select_kb()
    )
...
async def cb_select_full(call: CallbackQuery, state: FSMContext):
    if call.from_user.id != ALLOWED_USER:
        return
    await call.answer()
    await state.update_data(selected=list(range(1, count_lotties() + 1)))
    await call.message.edit_text(
...
async def ask_ba(msg: Message, state: FSMContext):
    ...
    if t.lower() != "skip":
        try:
            ...
            await msg.answer("⚠️ Invalid HEX color.")
            return
    else:
...
async def got_json(msg: Message, state: FSMContext):
    ...
    if not (msg.document.file_name or "").endswith(".json"):
        await msg.answer("⚠️ Only .json files allowed!")
        return
...
async def got_svg(msg: Message, state: FSMContext):
    ...
    if not (msg.document.file_name or "").lower().endswith(".svg"):
        await msg.answer("⚠️ Only .svg files allowed!")
        return
    f = await bot.get_file(msg.document.file_id)
    buf = io.BytesIO()
    await bot.download_file(f.file_path, buf)
...
async def got_logo_c1(msg: Message, state: FSMContext):
    ...
    if t.lower() != "skip":
        try:
            ...
            await msg.answer("⚠️ Invalid HEX.")
            return
...
async def run_pack(
    ...
    me = await bot.get_me()
    name = (
        f"pk{''.join(random.choices(string.ascii_lowercase + string.digits, k=8))}_by_{me.username}"
    )
    ...
    if not LOTTIES_DIR.exists() or not any(LOTTIES_DIR.glob("*.json")):
        await stat.edit_text(
            f"❌ Error: <code>lotties/</code> folder is empty or missing.", parse_mode="HTML"
        )
        return
    # ── Check layers ──
    if not d.get("layers"):
        await stat.edit_text(
            "❌ Error: No logo/text layers found in state. Please restart with /start.",
            parse_mode="HTML",
        )
        return
    files = []
    for fp in sorted(LOTTIES_DIR.glob("*.json")):
        try:
            n = int(fp.stem)
            if n in set(d.get("selected", [])):
                files.append((fp, n))
        except:
            pass
    if not files:
        await stat.edit_text(
            "❌ Error: No matching lottie files found for selected indices.", parse_mode="HTML"
        )
        return
    created = False
    ok = 0
